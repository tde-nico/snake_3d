...
class Game(Ursina):
	...
	def create_map(self, size):
		Entity(model='quad', scale=size, position=(size//2,size//2,0), color=color.dark_gray)
		Entity(model=Grid(size, size), scale=size, position=(size//2,size//2, -0.01), color=color.white)
