...
class Snake:
	def __init__(self, MAP_SIZE):
		...
		self.directons = {'a':Vec3(-1,0,0),'d':Vec3(1,0,0),'w':Vec3(0,1,0),'s':Vec3(0,-1,0)}
	...
	def create_segment(self, pos):
		...
		Entity(model='sphere', color=color.green, position=pos).add_script(
	...
	def run(self):
		...
		if not self.frame_counter % self.speed:
			...
			for segment, pos in zip(self.segment_entities, self.segment_positions):
				segment.position = pos

	def control(self):
		for key in 'wasd':
			if held_keys[key] and self.permissions[key]:
				self.direction = self.directons[key]
